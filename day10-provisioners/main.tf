...
resource "aws_key_pair" "vishnukey" {
  key_name   = "vishnukey"  # Replace with your desired key name
...
resource "aws_instance" "server" {
 ...
 provisioner "local-exec" {
  ...
  # File provisioner to copy a file from local to the remote EC2 instance
   provisioner "file" {
    source      = "local-file.txt"  # Path to the local file
    destination = "/tmp/remote-file.txt"    # Replace with the path on the remote instance
  }
